...
class ClientSession:
    ...
    def _phase_dh_handshake(self) -> bool:
        try:
            ...
            _dh.info(f"handshake STS concluído com {self.addr} — segredo partilhado derivado via DH, canal cifrado pronto (AES-256-GCM, chaves via HKDF-SHA256)")
            print(f"[DH] handshake OK com {self.addr}")
            return True

        except Exception as e:
            _dh.error(f"handshake STS falhou com {self.addr}: {e}")
            print(f"[!] DH falhou: {e}")
            return False
    ...
    def _socket_reader(self) -> None:
        ...
        while True:
            try:
                ...
                _session.debug(f"socket fechado pelo cliente ({self.addr}): {e}")
            ...
            while True:
                ...
                if tag == "eof":
                    ...
                    try:
                        ...
                        _session.error(f"falhou push de '{msg.type}' para o cliente: {e}")
                        break
                    ...
                    try:
                        ...
                        _dh.info("renegociação DH concluída — nova epoch instalada, mensagens seguintes usam chave derivada do novo segredo partilhado")
                    except Exception as e:
